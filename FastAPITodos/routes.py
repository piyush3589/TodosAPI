...
@router.post("/", response_description='create a todo list', status_code=status.HTTP_201_CREATED,
             response_model=ListModel)
def create_list(request: Request, list: ListModel = Body(...)):
    list = jsonable_encoder(list)
    new_list_item = request.app.database["todo_backend"].insert_one(list)
    created_list_item = request.app.database["todo_backend"].find_one({
...
def show_list(request: Request):
    todos = list(request.app.database["todo_backend"].find(limit=50))
    return todos


@router.delete("/", response_description="delete a item from list")
def delete_list(id: str, request: Request, response: Response):
    delete_result = request.app.database["todo_backend"].delete_one({"_id": id})
...
def update_item(id: str, request: Request, list: ListUpdateModel = Body(...)):
    ...
    update_result = request.app.database["todo_backend"].update_one({"_id": id}, {"$set": listItems})
    ...
    if (
            updated_list_item := request.app.database["todo_backend"].find_one({"_id": id})
    ) is not None:
